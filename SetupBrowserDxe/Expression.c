  ...
  )
{
  ...
  for (UserInfoHandle = NULL;;) {
    ...
    LOG ((EFI_D_ERROR, "%a.%d UserInfoSize=%X\n", __FUNCTION__, __LINE__, UserInfoSize));
    UserInfo = (EFI_USER_INFO *) AllocatePool (UserInfoSize);
    if (UserInfo == NULL) {
      break;
    }
    LOG ((EFI_D_ERROR, "%a.%d UserInfo=%p\n", __FUNCTION__, __LINE__, UserInfo));
    ...
    while (RemainSize >= sizeof (EFI_USER_INFO_ACCESS_CONTROL)) {
      ...
      if (AccessControl->Type == EFI_USER_INFO_ACCESS_SETUP) {
        ...
        LOG ((EFI_D_ERROR, "%a.%d Guid=%g\n", __FUNCTION__, __LINE__, Guid));
        LOG ((EFI_D_ERROR, "%a.%d UserPermissionsGuid=%g\n", __FUNCTION__, __LINE__, UserPermissionsGuid));
        AccessControlDataSize = AccessControl->Size - sizeof (EFI_USER_INFO_ACCESS_CONTROL);
        LOG ((EFI_D_ERROR, "%a.%d AccessControlDataSize=0x%X\n", __FUNCTION__, __LINE__, AccessControlDataSize));
