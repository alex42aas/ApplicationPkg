...
FUNCTIION_KEY_SETTING gFunctionKeySettingTable[] = {
   ...
   // Https messages 
  {
    { 
/*
0x19b816e9, 0x4813, 0x4b09,\
 { 0xaa, 0x9a, 0x91, 0x5c, 0x91, 0x32, 0x2d, 0x4
*/      
      0x19b816e9, 
      0x4813, 
      0x4b09, 
...
#if 0
        if (QuestionValue->date.Year  == EfiTime.Year && 
...
#if 0      
        if (QuestionValue->time.Hour   == EfiTime.Hour &&
  ...
  )
{
  ...
  while (!IsNull (&Form->StatementListHead, Link)) {
    ...
    LOG((EFI_D_ERROR, "%a.%d QuestionId: 0x%X, Type: 0x%X\n", 
      __FUNCTION__, __LINE__, Question->QuestionId, Question->HiiValue.Type));
    ...
    if (HiiValue->Type == EFI_IFR_TYPE_STRING ||
        ...
        StrLen((CHAR16 *)Question->BufferValue)*/) {
      LOG((EFI_D_ERROR, "%a.%d --> {%s}\n", __FUNCTION__, __LINE__,
        (CHAR16 *) Question->BufferValue));
    
    if (HiiValue->Type == EFI_IFR_TYPE_STRING)
        CallbackValue = (EFI_IFR_TYPE_VALUE *)Question->BufferValue;
     else 
        CallbackValue = &HiiValue->Value;
#if 0    
      UnicodeSPrint((CHAR16*)Question->BufferValue, 
        (UINTN)(((Question->Maximum + 1) << 1) & (UINTN)(-1)), L"**test**");
#else
      ActionRequest = EFI_BROWSER_ACTION_REQUEST_NONE;
      ...
      if (ActionRequest == EFI_BROWSER_ACTION_HIDE)
        Question->QuestionFlags |= HIDE_STATEMENT_FLAG;
      else
        Question->QuestionFlags &= ~HIDE_STATEMENT_FLAG;
      ...
      HiiValue->Value.string = 
        NewString ((CHAR16 *) Question->BufferValue, FormSet->HiiHandle);
#endif
    }
