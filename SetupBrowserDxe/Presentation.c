...
{
  EFI_GUID Guid = {MULTIBOOT_FORM_GUID};
  EFI_GUID FpGuid = {0xf76e0a70, 0xb5ed, 0x4c38, 
                      { 0xac, 0x9a, 0xe5, 0xf5, 0x4b, 0xf1, 0x6e, 0x35}};

  if (CompareGuid (&mCurrentFormSetGuid, &Guid)) {
    return TRUE;
  }
  if (CompareGuid (&mCurrentFormSetGuid, &FpGuid)) {
    return TRUE;
  }
  return FALSE;
}
...
{
  EFI_STATUS Status;
  MULTIBOOT_PROTOCOL *pMultiboot;

  if (!isMultiBootFormSet ()) {
    return NULL;
  }
  ...
  if (pMultiboot->GetVersionStr) {
    return pMultiboot->GetVersionStr ();
  }
  return NULL;
}
...
/**
  Clear screen with specified text attribute.
  ...
  )
{
  ...
  ClearScreen (0, LocalScreen.RightColumn, 0, LocalScreen.BottomRow, KEYHELP_BACKGROUND);
    ...
    for (Line = (UINT8) LocalScreen.TopRow; Line < BANNER_HEIGHT + (UINT8) LocalScreen.TopRow; Line++) {
      ...
      for (Alignment = (UINT8) LocalScreen.LeftColumn;
        ...
        if (gBannerData->Banner[RowIdx][ColumnIdx] != 0x0000) {
          ...
          if (VerIdx == 0) {
            StrFrontPageBanner = GetModuleVersion ();
            if (StrFrontPageBanner) {
              PrintStringAt (
                LocalScreen.LeftColumn + (LocalScreen.RightColumn - LocalScreen.LeftColumn) * 2 / 3, 
                Line, 
                StrFrontPageBanner);
            }
            VerIdx += BANNER_HEIGHT + 1;
          }
          continue;
        }
  ...
  )
{
  ...
  if ((gClassOfVfr & FORMSET_CLASS_FRONT_PAGE) != FORMSET_CLASS_FRONT_PAGE) {
    ...
    if (Version) {    
      PrintStringAt (
        LocalScreen.LeftColumn + (LocalScreen.RightColumn - LocalScreen.LeftColumn) * 2 / 
        3,
  ...
  while (!IsNull (&Selection->Form->StatementListHead, Link)) {
    ...
    } else if ((Statement->QuestionFlags & HIDE_STATEMENT_FLAG) == HIDE_STATEMENT_FLAG) {
      Suppress = TRUE;
    }
