  ...
  )
{
  ...
  if (Timeout != 0) {
    ...
    do {
      ...
      Status2 = gRT->GetVariable (L"FormRefresh", &gEfiGlobalVariableGuid,
          NULL, &DataSize, &Flag);
      if (EFI_SUCCESS == Status2) {
        if (Flag) {         
          Flag = 0;
          Status2 = gRT->SetVariable (L"FormRefresh", &gEfiGlobalVariableGuid,
      ...
      } else if (!EFI_ERROR (Status) && Index == 0) {
        gBS->SignalEvent (gST->ConIn->WaitForKey);
        Status2 = gBS->CheckEvent(gST->ConIn->WaitForKey);
  ...
  )
{
  ...
  while (TRUE) {
    ...
    case CfUiReset:
      ...
      {
        EFI_GUID MultibootGuid = {0x543cd5fe, 0x1276, 0x443d, 
          {0x41, 0x42, 0x34, 0xc2, 0x32, 0xef, 0xa1, 0xe4}};
        if (CompareGuid(&Selection->FormSetGuid, &MultibootGuid)) {
          gNvUpdateRequired = FALSE;
        }
      }
