  ...
  )
{
  ...
  UINTN                   CurrentCursor, DisplayCursor;
  BOOLEAN                 CursorVisible;
  UINTN                   Minimum;
  UINTN                   Maximum;
  FORM_BROWSER_STATEMENT  *Question;
  BOOLEAN                 IsPassword;
  CHAR16                  *BegStr, *EndStr;
  BOOLEAN                 IsNumeric       = FALSE;
  BOOLEAN                 IsNumericAndDot = FALSE;
  ...
  if ( EFI_IFR_STRING_NUMERIC == (Question->Flags & EFI_IFR_STRING_NUMERIC) ) {
    DEBUG((EFI_D_INFO, "IsNumeric = TRUE for Question->QuestionId = %d", Question->QuestionId));
    IsNumeric = TRUE;
  } else if (EFI_IFR_STRING_NUMERIC_DOT == (Question->Flags & EFI_IFR_STRING_NUMERIC_DOT)) {
    IsNumericAndDot = TRUE;
  }

  if (GetStringWidth(StringPtr) == 2) {
    /* if string is empty then clean trash */
  ...
  BegStr = CurrentCursor > (DimensionsWidth - 2) ? 
    &StringPtr[CurrentCursor + 1 - (DimensionsWidth - 2)] : StringPtr;
  EndStr = &StringPtr[CurrentCursor];
...
#else
{
    CHAR16 *TmpPtr = BegStr;

    Count = 0;
    
    while (TmpPtr != EndStr) {
      BufferedString[Count] = *TmpPtr;
      Count++;
      TmpPtr++;
      if (IsPassword) {
        PrintChar (L'*');
  ...
  do {
    ...
    default:
      ...
      if (IsNumeric && (Key.UnicodeChar < '0' || Key.UnicodeChar > '9'))
        break;
        
      if (IsNumericAndDot && ((Key.UnicodeChar < '0' || Key.UnicodeChar > '9') && Key.UnicodeChar != '.'))
        break;

      if (!Printable(Key.UnicodeChar)) {
        break;
      }
      ...
      if ((UINTN)(EndStr - BegStr) < (DimensionsWidth - 3) && 
          (GetStringWidth (StringPtr) < ((Maximum + 1) * sizeof (CHAR16))) &&
          (Key.UnicodeChar != CHAR_BACKSPACE)) {
        if (EndStr < &StringPtr[Maximum]) {
          EndStr++;
        }
      }
      break;
    ...
    if (CurrentCursor > (UINTN)(EndStr - StringPtr)) {
      if (EndStr < &StringPtr[Maximum]) {
        BegStr++;
        EndStr++;
      }
    }
    if (CurrentCursor < (UINTN)(BegStr - StringPtr)) {
      if (BegStr > StringPtr) {
        BegStr--;
        EndStr--;
      }
...
#else
{
    ...
    while (TmpPtr != EndStr) {
      BufferedString[Count] = *TmpPtr;
      if (IsPassword && *TmpPtr != CHAR_NULL) {
        PrintChar (L'*');
      }
      Count++;
      TmpPtr++;
    ...
    DisplayCursor = CurrentCursor - (UINTN)(BegStr - StringPtr);
    
    gST->ConOut->SetCursorPosition (gST->ConOut, Start + DisplayCursor + 1, Top + 3);
  } while (TRUE);
