...
#include <Library/OpenSSLGostHash/gost89.h>
#include <Library/OpenSSLGostHash/gost_hash.h>
...
#define memset(Buffer, Val, Len) ZeroMem(Buffer, Len)
...
#if 0
  memcpy(tmp_data, data+2,30);
  memcpy(data, tmp_data, 30);
#else
...
ghosthash_len rshift8_helper(ghosthash_len *plen)
{
  byte *ptr = (byte*)plen;
...
int finish_hash(gost_hash_ctx *ctx,byte *hashval)
	...
	if (ctx->left)
		{
		memset((byte*)buf,0,32);
		memcpy((byte*)buf,ctx->remainder,ctx->left);
		hash_step(ctx->cipher_ctx,H,(byte*)buf);
		add_blocks(32,S,(byte*)buf);
		fin_len+=ctx->left;
		}
	memset((byte*)buf,0,32);
	bptr=buf;
    ...
    //memmove(&fin_len, (byte*)&fin_len + 8, 7);
    //fin_len &= 0x00FFFFFFFFFFFFFFUL;
    fin_len = rshift8_helper((ghosthash_len*)&fin_len);    
		//fin_len>>=8;
		};
	hash_step(ctx->cipher_ctx,H,(byte*)buf);
